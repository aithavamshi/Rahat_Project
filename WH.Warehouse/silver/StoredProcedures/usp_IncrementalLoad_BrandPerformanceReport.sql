...
BEGIN
    ...
    BEGIN TRY
        ...
        ---------------------------------------------------
           DELETE FROM silver.BrandPerformanceReport
        WHERE 
            -- Delete any rows where the systemId is not present in source table
            [systemId-2000000000] NOT IN (
                SELECT [systemId-2000000000] FROM [test_lh].[dbo].[GLBudgetEntry96]
            ); 
